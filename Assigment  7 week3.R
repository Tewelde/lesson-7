...
plot(mean2010)
## composite
comp <- brick(mean2000, mean2005,mean2010)
plot(comp)
plotRGB(comp, 3, 2, 1, stretch='hist')
...
## Calculate mean
mean_ndvi1 <- extract(tura,e1, fun=mean, na.rm = TRUE)
mean_ndvi2 <- extract(tura,e2, fun=mean, na.rm = TRUE)
## make data frame
t1 <- data.frame(  date = sceneinfo$date,  sensor = sceneinfo$sensor,  ndvi = as.numeric(mean_ndvi1),   zone = "region1")
t2 <- data.frame(  date = sceneinfo$date,  sensor = sceneinfo$sensor,  ndvi = as.numeric(mean_ndvi2),   zone = "region2")
ts <- rbind(t1,t2)
...
ggplot(data = ts, aes(x = date, y = ndvi, colour = zone)) + 
  geom_point() + 
  geom_line()
